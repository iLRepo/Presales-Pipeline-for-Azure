...
import { getAccounts, createAccount, createWorkshop } from "@/lib/api";
import { toast } from "sonner";

interface Props { open: boolean; onOpenChange: (v: boolean) => void; onCreated?: () => void; defaultAccountId?: string; }

export function NewWorkshopDialog({ open, onOpenChange, onCreated, defaultAccountId }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setStakeholder(""); setNewAccountName("");
  ...
  const submit = async () => {
    ...
    try {
      ...
      await createWorkshop({ account_id: acctId, stakeholder_contact: stakeholder || null });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>New Envisioning Workshop</DialogTitle></DialogHeader>
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label>Stakeholder Contact</Label>
            <Input value={stakeholder} onChange={(e) => setStakeholder(e.target.value)} placeholder="Name / email" />
          </div>
          <div className="text-xs text-muted-foreground">
            All workshops are Envisioning workshops. Track Part 101 / 201 / 301 completion as a checklist on the workshop detail.
          </div>
        </div>
        <DialogFooter>
