    ...
    <div className="p-6 space-y-4 max-w-6xl">
      ...
      <Section title="Post-Workshop Tracker" cols={1}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <Field label="Estimated Revenue (free text)"><Input value={w.est_revenue_text ?? ""} onChange={(e) => set({ est_revenue_text: e.target.value })} onBlur={() => save.mutate({ est_revenue_text: w.est_revenue_text || null })} placeholder="e.g. $50K-$100K, $165K (Beak)" /></Field>
          <Field label="Estimated Revenue Salesforce URL"><Input value={w.est_revenue_url ?? ""} onChange={(e) => set({ est_revenue_url: e.target.value })} onBlur={() => save.mutate({ est_revenue_url: w.est_revenue_url || null })} placeholder="https://mycompany.lightning.force.com/lightning/r/Opportunity/..." /></Field>
